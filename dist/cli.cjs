...
async function fetchSelection() {
  ...
  const filePath = import_node_path.default.join(process.cwd(), JSON_FILE);
...
var import_node_fs, import_node_path, import_server, import_stdio, import_types, DEV_SERVER_PORT, JSON_FILE;
var init_mcp = __esm({
  "src/mcp.ts"() {
    "use strict";
...
function persistSelection(sel) {
  const outputPath = import_node_path2.default.join(_projectRoot, ".yumyum_pickr.json");
  const data = {
    ...sel,
    _instructions: [
  ...
  try {
    import_node_fs2.default.writeFileSync(outputPath, JSON.stringify(data, null, 2), "utf-8");
...
function setupCursorMcp() {
  const dir = import_node_path2.default.join(_projectRoot, ".cursor");
  const configPath = import_node_path2.default.join(dir, "mcp.json");
  try {
    ...
    if (import_node_fs2.default.existsSync(configPath)) {
      config = JSON.parse(import_node_fs2.default.readFileSync(configPath, "utf-8"));
    }
    ...
    import_node_fs2.default.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf-8");
...
function setupVscodeMcp() {
  const dir = import_node_path2.default.join(_projectRoot, ".vscode");
  const configPath = import_node_path2.default.join(dir, "mcp.json");
  try {
    if (!import_node_fs2.default.existsSync(dir)) import_node_fs2.default.mkdirSync(dir, { recursive: true });
    let config = {};
...
var import_node_fs2, import_node_http, import_node_path2, _server, _selection, _projectRoot, _port;
